	...
	: wxFrame(NULL, wxID_ANY, titulo, wxDefaultPosition, size)
{
	...
	tcNombre = new wxTextCtrl(panel, ID_tcNOMBRE, _("Nombre..."), wxDefaultPosition, wxSize(150,25)); //Al primer TextCtrl le pongo tamaño para que todos se asapten a una longitud determinada)
	...
	Connect(ID_bACEPTAR, wxEVT_COMMAND_BUTTON_CLICKED, wxCommandEventHandler(FrameNuevo::onAceptarClick));
	Connect(ID_tcNOMBRE, wxEVT_KEY_DOWN, wxKeyEventHandler(FrameNuevo::OnChar));
}
//----------------------------------------------------------
...
//----------------------------------------------------------
void FrameNuevo::onAceptarClick(wxCommandEvent& event){
	wxBell();
	Hide();
}
///----------------------------------------------------------
...
//---------------------------------------------------------
void FrameNuevo::OnChar(wxKeyEvent& event)
{
